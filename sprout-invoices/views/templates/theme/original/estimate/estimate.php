					...
					<div id="doc_actions">
						...
						<?php else : ?>
							<?php if ( ! si_is_estimate_approved() ) : ?>
								<a href="#accept" class="button primary_button status_change" data-status-change="accept" data-id="<?php the_ID(); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( SI_Controller::NONCE ) ); ?>"><?php esc_html_e( 'Accept Estimate', 'sprout-invoices' ); ?></a>
							<?php else : ?>
							...
							<?php if ( ! si_is_estimate_declined() ) : ?>
								<a href="#decline" class="button status_change" data-status-change="decline" data-id="<?php the_ID(); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( SI_Controller::NONCE ) ); ?>"><?php esc_html_e( 'Decline Estimate', 'sprout-invoices' ); ?></a>
							<?php else : ?>
		...
		<div id="doc_history">
			...
			<?php foreach ( si_doc_history_records() as $item_id => $data ) : ?>
				<dt>
					...
					<span class="history_date"><?php echo esc_html( gmdate( get_option( 'date_format' ) .' @ '. get_option( 'time_format' ), strtotime( $data['post_date'] ) ) ); ?></span>
				</dt>
				<?php
