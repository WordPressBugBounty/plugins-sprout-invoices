		...
		<div>
			<?php foreach ( $attachments as $media_id ) : ?>
				<?php
					...
					?>
				<span>
					<img src="<?php echo esc_url( $icon ) ?>" title="<?php echo esc_html( get_the_title( $media_id ) ) ?>" class="doc_attachment attachment_type_<?php echo esc_attr( $filetype['ext'] ) ?>" style="width: 40px; height: auto;">
				</span>
			<?php endforeach ?>
		</div>
	</div>
